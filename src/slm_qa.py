...
def response_generator_node(state: PlantQAState) -> PlantQAState:
    """응답 생성 노드 - DB, RAG, 웹 검색 결과를 종합하여 답변 생성 (lm.invoke(str) 대응 완료)"""
    
    def is_valid_result(text: str) -> bool:
        """검색 결과가 부실하거나 환각일 경우 무시"""
        if not text:
            return False
        t = text.strip()
    ...
    # 답변 생성
    try:
        response = lm.invoke(prompt)  # str 반환
        state["final_response"] = response.strip()  # .content 제거
    except Exception as e:
